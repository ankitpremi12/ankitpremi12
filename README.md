<!-- SNAKE ANIMATION - flowing contribution graph at top -->
<div align="center">
  <img src="https://raw.githubusercontent.com/ankitpremi12/ankitpremi12/output/github-contribution-grid-snake-dark.svg" alt="Snake animation" />
</div>

---

<!-- HEADER -->
<div align="center">

```
╔═══════════════════════════════════════════════════════════════╗
║                                                               ║
║     ░█████╗░███╗░░██╗██╗░░██╗██╗████████╗                    ║
║     ██╔══██╗████╗░██║██║░██╔╝██║╚══██╔══╝                    ║
║     ███████║██╔██╗██║█████═╝░██║░░░██║░░░                    ║
║     ██╔══██║██║╚████║██╔═██╗░██║░░░██║░░░                    ║
║     ██║░░██║██║░╚███║██║░╚██╗██║░░░██║░░░                    ║
║     ╚═╝░░╚═╝╚═╝░░╚══╝╚═╝░░╚═╝╚═╝░░░╚═╝░  P R E M I          ║
║                                                               ║
║          [ AI/ML ENGINEER · GENAI · NLP · MLOPS ]            ║
╚═══════════════════════════════════════════════════════════════╝
```

<a href="mailto:ankitpremiji@gmail.com"><img src="https://img.shields.io/badge/✉ EMAIL-0a0a0a?style=for-the-badge&logoColor=white"/></a>
<a href="https://www.linkedin.com/in/ankit-premi-a37b37255/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="https://github.com/ankitpremi12"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/></a>
<img src="https://komarev.com/ghpvc/?username=ankitpremi12&style=for-the-badge&color=blueviolet&label=PROFILE+VIEWS"/>

</div>

---

<img align="right" width="380" src="https://github-readme-stats.vercel.app/api?username=ankitpremi12&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=58a6ff&icon_color=58a6ff&text_color=c9d1d9"/>

### 🧬 `whoami`

```python
class AnkitPremi:
    role       = "AI/ML Engineer"
    focus      = ["GenAI", "LLMs", "RAG", "MLOps"]
    experience = ["Vodafone", "C-DOT (Govt. of India)"]
    education  = "B.Tech AI & DS @ MAIT Delhi · 8.5 GPA"
    status     = "🟢 Open to Work"
    superpower = "Turning messy data → production AI"
```

> *"I don't just build models — I ship systems that work in the real world."*

<br clear="right"/>

---

### 🏢 Where I've Built Things

```
▸ VOIS (Vodafone Intelligent Solutions)          Sep – Oct 2025
  └─ Built RAG + LLM enterprise reporting system
  └─ Automated workflows for 50+ stakeholders via Slack
  └─ Tech: Python · GPT-4 · Claude · RAG · REST APIs

▸ Centre for Development of Telematics (C-DOT)   Jul – Aug 2025
  └─ Cybersecurity ML system for phishing detection
  └─ 45+ engineered features · RF + XGBoost ensemble
  └─ Tech: Python · XGBoost · Scikit-learn · MLOps
```

---

### 🔭 Projects That Slap

<table>
<tr>
<td width="50%">

**🛡️ SLD Threat Classifier**
> Detects typosquatting & homoglyph attacks in real-time

`XGBoost` `Unicode NLP` `Regex` `Scikit-learn`

✅ 89% precision · 86% recall
✅ 45+ custom engineered features
✅ Homoglyph + typosquat dual detection

[→ View Project](https://github.com/ankitpremi12/Enhanced-SLD-Threat-Classifier-with-Typosquat-Homoglyph-Detection)

</td>
<td width="50%">

**🤖 Agentic AI Analytics Assistant**
> Autonomous BI agent with dynamic task planning

`LangChain` `RAG` `VectorDB` `LLMs`

✅ 95% retrieval accuracy
✅ 60% faster vs manual analysis
✅ Multi-source vector + structured data

</td>
</tr>
<tr>
<td width="50%">

**📈 Customer Churn Prediction**
> End-to-end churn prediction with NLP explainability

`XGBoost` `FastAPI` `RFM Analysis` `RAG`

✅ 87% accuracy · 0.82 AUC-ROC
✅ 30+ behavioral features
✅ 1000+ req/day · <200ms latency

</td>
<td width="50%">

**🦠 Disease Spread Simulation**
> Epidemic modeling on large-scale networks

`NetworkX` `BFS/DFS` `Plotly` `NumPy`

✅ 82% accuracy vs real-world data
✅ 10K+ node simulation
✅ 70% performance optimization

</td>
</tr>
</table>

---

### 🧰 Arsenal

<div align="center">

**🤖 GenAI & LLMs**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![OpenAI](https://img.shields.io/badge/GPT--4-412991?style=flat-square&logo=openai&logoColor=white)
![Claude](https://img.shields.io/badge/Claude-CC785C?style=flat-square&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logoColor=black)
![LlamaIndex](https://img.shields.io/badge/LlamaIndex-7C3AED?style=flat-square)

**🧠 ML / DL**

![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-189AB4?style=flat-square)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)

**☁️ Cloud & MLOps**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![GCP](https://img.shields.io/badge/GCP-4285F4?style=flat-square&logo=googlecloud&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=flat-square&logo=mlflow&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)

**🗄️ Databases**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Pinecone](https://img.shields.io/badge/Pinecone-000000?style=flat-square)
![FAISS](https://img.shields.io/badge/FAISS-0467DF?style=flat-square)
![Weaviate](https://img.shields.io/badge/Weaviate-00A98F?style=flat-square)

</div>

---

### 📊 GitHub Activity

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ankitpremi12&theme=tokyonight&hide_border=true&background=0d1117" width="70%"/>
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=ankitpremi12&theme=tokyo-night&hide_border=true&bg_color=0d1117" width="100%"/>
</div>

---

### 🏅 Certifications

<div align="center">

`🧠 GenAI Essentials` **Microsoft & LinkedIn** &nbsp;·&nbsp; `☁️ Vertex AI Gemini` **Google Cloud**

`🤖 ML: Regression & Classification` **DeepLearning.AI** &nbsp;·&nbsp; `📊 BigQuery Analytics` **Google Cloud**

`📐 Statistics for Data Science` **Stanford University** &nbsp;·&nbsp; `🐍 Python` **Google**

</div>

---

<div align="center">

### 💼 Open to Roles in

**`AI/ML Engineering`** &nbsp;·&nbsp; **`Data Science`** &nbsp;·&nbsp; **`GenAI Development`** &nbsp;·&nbsp; **`MLOps`**

<br>

**Let's build something intelligent together.**

<a href="mailto:ankitpremiji@gmail.com"><img src="https://img.shields.io/badge/📩 Hire Me-FF6B35?style=for-the-badge"/></a>
<a href="https://www.linkedin.com/in/ankit-premi-a37b37255/"><img src="https://img.shields.io/badge/Connect on LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>

<br>

```
"The best AI system is the one that solves a real problem, not just scores high on a benchmark."
```

![Wave](https://raw.githubusercontent.com/mayhemantt/mayhemantt/Update/svg/Bottom.svg)

</div>
